...
artifact and dependency record, exact execution graph, outcome,
`ContainmentClosure`, and evidence digests. It has
...
- `executable`, `interpreter`, `script`, `runtime_helper`, `loaded_bundle`,
  `fixture_tree`, `read_only_resource`, `static_link_input`,
  `dynamic_link_input`, `system_tool`, and the explicitly trusted
  `bootstrap_authority`;
...
Every selected runtime path and every fixture/resource path maps to exactly one
staged node; every node is reachable from one selected root; every required edge
is present; and no verified-but-unused node may substitute for an executed node.
Fixture trees and read-only resources are copied or descriptor-cloned into the
run-owned staging tree, hashed completely, opened only through staged
descriptor-relative paths, and rechecked at the pre-launch barriers. Symlinks
must remain inside the staged fixture root. Bash, Node, Python, CMake, CTest,
compilers, linkers, OS loaders, and SDKs are explicit
system-tool/platform-boundary nodes with realpath, digest, version, dynamic
dependency, and OS/SDK identity.

`bootstrap_authority` is the sole deliberate exception: the broker's
installation-root bootstrap sentinel is trusted by the independently ratified
broker-generation anchor, uses no candidate run resource, and is started before
allocation. It is still recorded with executable digest and schema/provenance
identity; the exception does not permit an ambient product artifact or fixture.
...
identity, and a generated-output contract digest. That contract digest is
computed from generated outputs after replacing embedded bundle-ID and trust-
anchor fields with fixed zero sentinels; it never includes final generated bytes
that contain the bundle ID. Its own ID uses the same body-minus-self
domain-separated rule. Generation therefore has no cryptographic fixed point:
compute the bundle ID, embed it, then hash the final validator for the
independent trust anchor. Every generated validator and writer embeds the
accepted bundle ID; every document names its schema and bundle IDs.
...
bootstrap and contains the ratified current bundle ID, the final generated
validator output digest, and an explicit compatibility allowlist. The launcher
also contains a fixed raw 32-byte `SchemaTrustAnchorId` constant in code owned
outside the generated schema directory. The bootstrap parser for this tiny
anchor is independent of the generated schema validator. Validation order is
fixed: the root launcher checks the anchor's canonical bytes and exact anchor ID
against that constant, computes the candidate bundle ID with the fixed
bootstrap algorithm, compares it to the anchor, verifies the generated
validator's embedded bundle ID and final output digest against the anchor, and
only then invokes that validator for ordinary documents. A stale schema plus
matching stale validator and stale anchor is rejected by a current launcher even
when all old content-addressed objects remain stored. A compatibility or
rollback entry is accepted only when explicitly present in the independently
ratified anchor and covered by a downgrade test. The anchor and its launcher
constant change only in an independently reviewed bootstrap commit and are
included in ownership and baseline manifests.
...
```text
SHA-256("daw-doc-v1\0" || schema_id_raw_32_bytes || "\0" ||
        JCS(document_without_id))
```

`schema_id_raw_32_bytes` is the 32-byte SHA-256 digest, never its hexadecimal
rendering. All implementations use those exact bytes and the literal ASCII
domain separator above.
...
that candidate. Ambiguous ownership quarantines the candidate; it is never
guessed or broadly swept. A collision transitions the journal to `aborted`,
releases only those exact candidate reservations, and mints a new run ID. TCP
endpoints are intentionally not values in RunContext:
...
pipe, or staged product artifact; it uses only the pre-existing broker control
root and a fixed broker-generation identity. It acquires the active-run lease and
initializes the containment journal before any candidate allocation. Only after
that handoff does it stage the per-run lifecycle authority and start the service
supervisor. The supervisor owns product services and endpoint readiness. Neither
product code nor the supervisor can release or impersonate the lifecycle
authority.
...
the trusted per-user root and an OS-user-scoped singleton startup lease. Its
`BrokerGenerationIdentity` contains the broker executable/artifact digest,
accepted `SchemaTrustAnchorId`, protocol fingerprint, and AssemblyContext
identity. The checkout-local launcher sends its selected InvocationContext and
requires an authenticated handshake proving those exact values before using an
existing generation. A broker from another worktree, schema bundle, or product
revision is refused even if its OS peer credentials and generation name match.
If none answers, the startup-lease winner creates a new generation and
atomically publishes the per-user broker pointer; it never reuses or unlinks the
stale endpoint. Old generation directories are later handled by the same
registry/GC transaction.
...
evidence. It retains the active-run lease until that closure is durable and a
...
`ContainmentClosure`, and only then select the terminal result. If broker and
sentinel both die, the next launcher or user-invoked recovery/GC command follows
the same path. Two claimants race the active-run recovery lease, result-writer
lease, and terminal selector; only the winners may mutate the recovery journal
or publish. A terminal selector is the authoritative referent for GC and status;
result documents not selected by it are never eligible as the run's terminal
result.
...
registered process identity and its selected `RunTerminalRecord` is present.
On POSIX, recovery
...
lease without a gap until containment is closed, the journal is closed, and a
`RunTerminalRecord` is published. A held lease means active. An unlocked lease with no
...
- `control/` contains sockets, capabilities, active pointers, containment
  membership journals, allocation journals, and other ephemeral lifecycle state
  removed only after a valid `ContainmentClosure` and `RunTerminalRecord`;
...
`PayloadIndex` identifies every payload referenced by the selected RunResult and
...
candidate's run/CAS lease nonblocking. Acquiring an abandoned run lease changes
  ...
  publish the terminal selector before recomputing eligibility. This closes
...
The wider migration owns all 106 tracked scripts containing the hard-coded
`BUILD="$ROOT/build"` assignment, all 18 scripts containing the override-aware
`DAW_BUILD_DIR` assignment, the separate `build-tsan` script, and the Rust e2e,
bridge, web-stack, and discovery entry points. The six currently unregistered
surfaces are explicitly included: `tools/extraction_cost.sh`,
`tools/nc_unit_build.sh`, `tools/rt_load_probe.sh`,
`tools/demo_rehearsal.sh`, `tools/plugin_insert_stall_repro.sh`, and
`tools/undo_session_state_check.sh`. Their exact tracked paths are generated
into the ownership manifest; a count or glob is not ownership.
...
| Schema fixed-point | Regenerate bundle and validator with embedded IDs | Zero-sentinel contract digest makes bundle ID and final validator digest deterministic and non-circular |
...
| Document ID vectors | Compute IDs in C++, Rust, and TypeScript | Raw 32-byte schema digest and exact domain-separated preimage produce identical IDs |
...
| Fixture mutation | Change a declared fixture or read-only resource after staging and before launch | Staged descriptor/tree digest or pre-launch barrier refuses; no ambient fixture is read |
...
| Allocation frame tail | Tear a frame or head update at every transition, including committed-before-context | Valid frame prefix and head determine one state; recovery completes or rolls back deterministically |
...
| Broker generation skew | Keep an authenticated broker from another worktree/schema revision | Launcher refuses before InvocationContext or RunResult authority is delegated |
...
| Terminal selector race | Crash after candidate result staging and race two terminal publishers | One `RunTerminalRecord` wins; equal tuple is idempotent, different tuple quarantines; only selected result is authoritative |
