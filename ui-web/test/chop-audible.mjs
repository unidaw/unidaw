...
if (!existsSync(WAV)) {
  ...
  if (runs.length < 2) {
    ...
    // From each ONSET, so window k of one hit is compared with window k of the other.
    ...
    soundCheck(diff / scale > 0.15,
          ...
          `mean envelope difference ${(diff / scale).toFixed(3)} of peak, from their onsets`);
  }
}
